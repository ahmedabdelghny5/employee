...
import fileModel from "../../DB/model/files.model.js";
...
export const deleteFile = asyncHandler(async (req, res, next) => {
    ...
    if (!idFiles || idFiles == '') {
        ...
        if (!user.publicId.length) {
            return next(new AppError("There's no files  !", 404));
        }
        for (let i = 0; i < user.publicId.length; i++) {
            if (user.publicId[i] == idFiles) {
                ...
                if (deleted.result != 'ok') {
                    return next(new AppError("Error Deleting file Picture !", 403));
                } else {
...
export const addFiles = asyncHandler(async (req, res, next) => {
    if (!req.file) {
        return next(new AppError("plz enter your file", 400));
    }
    const user = await userModel.findById({ _id: req.user.id });
    if (!user) {
    ...
    const newUser = new fileModel({ file: fileUrl, createdBy: req.user.id })
    const savedUser = await newUser.save()
    savedUser ? res.json({ msg: "done", savedUser }) : next(new AppError("fail", 500));
})


export const deleteFilePicFn = asyncHandler(async (req, res, next) => {
    const { fileId } = req.params;
    const fileCheck = await fileModel.findByIdAndDelete({ _id: fileId, createdBy: req.user.id });
    if (!fileCheck) {
        return next(new AppError("There's no file or not owner !", 401));
    }
    fs.unlink(fileCheck.file, (err) => {
        if (err) {
            return next(new AppError(`Error with removing old file: ${err}`, 400));
        }
    })
